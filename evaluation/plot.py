...
def plot_arp(df, dataset, title, measures=MEASURES, symbols=["-P","-o","-v","-s", "-d"], sorted_columns=False, legend=False, save=False):
    # Data
    ...
    for n, measure in enumerate(measures):
        if sorted_columns:
            res = df.pivot_table(index='method', columns='subcollection', values=f"ARP_{measure}")[sorted_columns].T
        else:
            res = df.pivot_table(index='method', columns='subcollection', values=f"ARP_{measure}").T
...
def plot_per_topic(df, dataset, method, measure, subcollections, cut_off=1000, save=False):
    # data
    ...
    if save:
        plt.savefig(f"../../../paper/ECIR23/figures/ARP_per_topic_{dataset}_{method}_{measure}.png", bbox_inches='tight')
    plt.show()


def plot_per_topic_dif(df, dataset, method, measure, subcollections, cut_off=1000, save=False):
    # data
    data = defaultdict(lambda: dict(zip(subcollections, [0 for _ in range(len(subcollections))])))
    for subcollection in subcollections:
        per_topic = df[(df["dataset"]==dataset)&(df["subcollection"]==subcollection) & (df["method"]==method)].iloc[0]["arp_per_topic"]
    ...
    if save:
        plt.savefig(f"../../../paper/ECIR23/figures/ARP_per_topicdiff_{dataset}_{method}_{measure}.png", bbox_inches='tight')
    plt.show()


def plot_per_topic_delta(df, dataset, method, measure, subcollections, cut_off=1000, save=False):
    ...
    if save:
        plt.savefig(f"../../../paper/ECIR23/figures/ARP_per_topic_delta_{dataset}_{method}_{measure}.png", bbox_inches='tight')
    plt.show()
...
def table_ARP_full(long, highlight=True, save=False):
    # data
    ARP_table = long[(long["group"]=="ARP") & (long["measure"].isin(MEASURES))].pivot(index=["measure", "method"], columns=["dataset", "subcollection"], values="value").sort_index(axis=1)
    ...
    # export
    if save:
        ARP_table.to_latex(f"../../../paper/ECIR23/tables/ARP_full.tex", convert_css=True)
...
def table_RD_DRI(long, highlight=True, save=False):
    # data
    df = long[(long["group"].isin(["RD", "DRI"])) & (long["measure"].isin(MEASURES)) & (~long["subcollection"].isin(["WT", "t1", "round1"])) & (long["method"]!= "BM25")].pivot(index=["measure", "method"], columns=["dataset", "subcollection", "group"], values="value").sort_index(axis=1)
    ...
    if highlight:
        ...
        for index in df.index.levels[0]:
            for col in df.columns:
                ...
                df_styled = df_styled.background_gradient(cmap="Greens", subset=(index, col), vmin=min, vmax=max)
        df = df_styled
    
    # export
    if save:
        df.to_latex(f"../../../paper/ECIR23/tables/RD_DRI.tex", convert_css=True)
...
def _plot_DRI_ER(long, dataset, subcollection, measures=MEASURES, save=False):
    ...
    for measure, mk in zip(measures, marker_color):
        # data
        er = long[(long["dataset"] == dataset)&(long["measure"]==measure)&(long["group"]=="ER")&(long["subcollection"]==subcollection)]["value"].clip(0,2)
        dri = long[(long["dataset"] == dataset)&(long["measure"]==measure)&(long["group"]=="DRI")&(long["subcollection"]==subcollection)]["value"].clip(-0.1,0.1)
    ...
    for measure in measures:
        for method in methods: 
            texts.append(ax.text(
                x = long[(long["dataset"] == dataset)&(long["measure"]==measure)&(long["group"]=="ER")&(long["subcollection"]==subcollection)&(long["method"]==method)]["value"].clip(0,2).values[0],

                y = long[(long["dataset"] == dataset)&(long["measure"]==measure)&(long["group"]=="DRI")&(long["subcollection"]==subcollection)&(long["method"]==method)]["value"].clip(-0.1,0.1).values[0],
                s = method))
    ...
    if save:
        plt.savefig(f"../../../paper/ECIR23/figures/ER_DRI_{dataset}-{subcollection}.png", bbox_inches='tight')
    plt.show()
